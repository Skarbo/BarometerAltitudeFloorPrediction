...
public class QueryBuilder {
	...
	public Cursor query(Context context, Uri uri) {
		...
		return context.getContentResolver().query(uri, projection, builtSelection.first, builtSelection.second,
				sortOrder);
	}
	...
	/**
	 * Updates any rowh that matches the defined selection, with the specified
	 * values.
	 * 
	 * @param context
	 * @param values
	 ...
	 * {@link ContentProvider#applyBatch(ArrayList)}.
	 * 
	 * @return
	 */
	public ContentProviderOperation createUpdateOperation(ContentValues values, Uri uri) {
		Pair<String, String[]> builtSelection = buildSelection();
		return ContentProviderOperation.newUpdate(uri).withSelection(builtSelection.first, builtSelection.second)
	...
	public ContentProviderOperation createDeleteOperation(Uri uri) {
		Pair<String, String[]> builtSelection = buildSelection();
		return ContentProviderOperation.newDelete(uri).withSelection(builtSelection.first, builtSelection.second)
				.build();
	}
